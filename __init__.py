...
def io_import(fname, dmc):
	...
	for i in data:
		...
		setattr(eval(dmc),i, exec_line)
		#print("eval(dmc, i, exec_line", eval(dmc), i, exec_line)


def io_export(filename, dmc):
	print("saving dmc: filename, dcm", filename, dmc)
	items, types = plugin_get(dmc)
	dmc = 'bpy.context.scene.vray.' + dmc
	directory = config_path_get() + "vrayblender\\presets\\gi"
...
class MenuCopy(bpy.types.Menu):
	...
	def draw(self, context):
		...
		col = layout.column
		for i in range(1, 3):
			menu = row.operator('exec.rendersettingscopy', text="Copy current render settings to Cam" + str(i),
				icon='VRAY_LOGO')
			menu.index = i
			row = layout.row()
...
class NodePanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		for i in range(1, 3):
			...
			row.prop_search(bpy.context.scene, "ObjCam" + str(i), bpy.context.scene, "objects", text='',
				icon='CAMERA_DATA')  # , text ="Cam"+str(i))

			render = row.operator('exec.render', text="", icon='RENDER_STILL')
			render.renderindex = i
			render = row.operator('exec.setactivecamera', text="",
				icon='REC' if NodePanel.renderindex == i else 'RADIOBUT_OFF')
			render.renderindex = i
			row = layout.row()
